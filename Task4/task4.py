...
class Edrone():
	...
	* Example Call: e_drone.initial_pid()
	...
	def initial_pid(self):
	    ...
	    # if compute_path_flag%2 == 0:
	    # 	compute_path_flag -= 1
	    # elif compute_path_flag == 0:
	    # 	compute_path_flag = 1
	    # else:	    
	...
	* Example Call: e_drone.drone_shutdown()
	...
	def drone_shutdown(self):
		...
		while self.drone_position[2] < 30:
	...
	def travel_on_path():
		while compute_path_flag != 7
			self.path_flag_pub.publish(self.compute_path_flag)

			while(self.waypoints_flag!=1):
				self.pid()

			for ix in range(len(self.waypoints)):
				# print ("Waypoint no-", ix)
				# Setting waypoints as Setpoint(destination) for drone
				self.setpoint[0] = self.waypoints[ix][0]
				self.setpoint[1] = self.waypoints[ix][1]
				self.setpoint[2] = self.waypoints[ix][2]
				self.calc_error()
				while(abs(self.error[0])>=0.5 or abs(self.error[1])>=0.5 or abs(self.error[2])>=5):
				#for k in range(20):
			...
			self.compute_path_flag = self.compute_path_flag + 1
			self.waypoints_flag = 0
			self.waypoints = []

		if self.compute_path_flag>=8:
			# e_drone.initial_waypoint = e_drone.secure_initial_waypoint
			self.setpoint = self.secure_initial_waypoint
			# print "-------"
	...
	def start_run():
		# Arming the Drone
		self.arm()
		# Only one time arm because two times will decrease path flag twice
		# e_drone.arm()
